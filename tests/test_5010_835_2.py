...
class TestParsed835(unittest.TestCase):
    def setUp(self):
        with open('tests/5010-835-example-2.txt') as f:
    ...
    def test_financial_information(self):
        ...
        self.assertEqual(fi.amount, Decimal('100.00'))
    ...
    def test_payer(self):
        ...
        self.assertEqual(cd.name, 'PAYER NAME')
    ...
    def test_claims(self):
        self.assertEqual(len(self.facade.claims), 2)

    def test_claim_1(self):
        claim = self.facade.claims[0]
        self.assertEqual(claim.date_received, datetime.date(2011, 9, 7))
        self.assertIsNone(claim.date_statement_period_start)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            'EDI PAID')
        self.assertEqual(claim.patient.first_name, 'FIRST NAME')
        self.assertIsNone(claim.group_or_policy_number)
        self.assertIsNone(claim.contract_class)

        self.assertEqual(len(claim.line_items), 1)

        line_item = claim.line_items[0]
        self.assertEqual(line_item.charge, Decimal('2083'))
        self.assertEqual(line_item.payment, Decimal('100'))
        self.assertEqual(line_item.quantity, '1')
        self.assertEqual(line_item.provider_control_number, '1')
        self.assertEqual(line_item.allowed_amount, Decimal('100'))
        self.assertEqual(
