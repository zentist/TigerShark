...
class TestParsed835(unittest.TestCase):
    def setUp(self):
        with open('tests/5010-835-example-3.txt') as f:
    ...
    def test_financial_information(self):
        ...
        self.assertEqual(fi.amount, Decimal('57.44'))
    ...
    def test_claim_1(self):
        ...
        self.assertEqual(
            ...
            'EDI DENIAL')
        self.assertEqual(claim.patient.first_name, 'FIRST')
        self.assertEqual(claim.group_or_policy_number, '123456')
        self.assertIsNone(claim.contract_class)

        self.assertEqual(len(claim.line_items), 1)

        line_item = claim.line_items[0]
        self.assertEqual(line_item.charge, Decimal('1088'))
        self.assertEqual(line_item.payment, Decimal('0'))
        self.assertEqual(line_item.quantity, '8')
        self.assertIsNone(line_item.provider_control_number)
        self.assertEqual(line_item.allowed_amount, Decimal('0'))
        self.assertEqual(
